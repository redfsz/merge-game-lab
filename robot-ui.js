import {D,state,$,OS,COLORS,NAMES,ICONS,byName,byDay,fmt,esc,ds,glyph,toast,render,renderContent,itemDetail,itemLineText} from './app.js';
...
function paint(){
  ...
  document.querySelectorAll('[data-robot-cell]').forEach(b=>{
    ...
    if(b.dataset.content!==key){
      ...
      const name=g?osLabel(g)+'生成器':item?.display??'空位';const description=item?`${name} · ${itemLineText(item)}`:name;b.title=description;b.setAttribute('aria-label',`${description}，${Math.floor(i/7)+1}行${i%7+1}列`);
    }
...
function paintMonitor(){
  const energyInput=$('#robot-energy-input');if(energyInput&&document.activeElement!==energyInput)energyInput.value=sim.energy;
...
function bind(){
  ...
  $('#robot-energy-input').onfocus=stopRobot;
  $('#robot-energy-apply').onclick=()=>{const field=$('#robot-energy-input');if(!field.value.trim()||!field.checkValidity()){toast('体力请输入0到1000000的整数');return}const value=Number(field.value);manual(()=>{const r=sim.setEnergy(value);if(r.ok)sim.record('体力设置',r.message,'手动配置当前剩余体力，保留棋盘和订单，不计入生成或加工消耗');return r})};
  $('#robot-energy-input').onkeydown=e=>{if(e.key==='Enter'){$('#robot-energy-apply').click()}};
