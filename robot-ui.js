...
function paintMonitor(){
  $('#robot-monitor').innerHTML=`<div><b>${fmt(sim.spent)}</b><small>消耗体力 ${help('energy')}</small></div><div><b>${sim.actions}</b><small>操作步数 ${help('step')}</small></div><div><b>${sim.merges}</b><small>合成次数 ${help('level')}</small></div>`;
  $('#robot-time').innerHTML=`操作用时 ${clock(sim.elapsed)} ${help('time')}`;$('#robot-cd-time').textContent=cdTimeLabel();
...
function showTrace(step){stopRobot();const point=sim.energyTrace.find(p=>p.step===step),events=sim.events.filter(e=>e.step===step);$('#detail-body').innerHTML=`<h2>步骤 ${step} ${help('step')}</h2><div class="trace-summary"><span>剩余体力 <b>${fmt(point?.value)}</b> ${help('energy')}</span></div>${eventHtml(events)||'<p class="workflow-frequency">这是本局的初始体力。</p>'}`;if(!$('#detail').open)$('#detail').showModal()}
function paintWorkflowLog(){if(!$('#workflow-event-list'))return;$('#workflow-event-list').innerHTML=eventHtml(sim.events.slice(-8).reverse())||'<p class="workflow-frequency">点播放，先按计划挑选一张订单。</p>';$('#full-workflow-log').onclick=()=>{$('#detail-body').innerHTML=`<h2>行为与原因 ${help('step')}</h2><p class="workflow-frequency">共${sim.events.length}条；最近的记录在前。</p><button id="export-workflow-log">导出完整日志</button><div class="workflow-event-list" style="max-height:65vh">${eventHtml(sim.events.slice(-300).reverse())}</div><p class="workflow-frequency">窗口展示最近300条，导出包含本局全部记录。</p>`;$('#export-workflow-log').onclick=()=>{const blob=new Blob([JSON.stringify(sim.events,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`robot-log-day-${state.day}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)};if(!$('#detail').open)$('#detail').showModal()}}
