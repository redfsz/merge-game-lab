...
import {RobotSimulator,RobotPlayer,STRATEGIES} from './robot.js';
...
export function resetRobot(){
  ...
  let options={opening:opening==='carry'?'carry':'empty',strategy:policy,focus};
...
export function renderBoard(){
  ...
  $('#content').innerHTML=`<div class="robot-lab ${editing?'editing':''}"><div class="robot-top"><h1>🤖 机器人游乐场</h1><div class="robot-top-actions"><label>日期<select id="robot-day" aria-label="模拟日期">${ds().map(d=>`<option value="${d.day}" ${d.day===state.day?'selected':''}>第 ${d.day} 天</option>`).join('')}</select></label><button id="edit-opening">${editing?'取消布置':'▦ 布置初始棋盘'}</button><button class="small ghost" id="robot-help" aria-label="查看模拟规则">ⓘ</button></div></div><div class="robot-layout"><section class="robot-stage"><div class="robot-stage-head"><div class="robot-state"><span class="robot-avatar">${editing?'🧩':'🤖'}</span><div><strong id="robot-status"></strong><small id="robot-current-action"></small></div></div><div id="energy-pill" class="robot-energy">ϟ <b id="robot-energy"></b><small>体力</small></div></div>${editing?'<div class="editor-notice"><span>选物品 → 点格子放入</span><b id="editor-count"></b></div>':''}<div class="board-surface"><div class="board-grid" id="robot-grid" role="group" aria-label="7列8行合成棋盘">${Array.from({length:56},(_,i)=>`<button class="tile" data-robot-cell="${i}" aria-label="第${Math.floor(i/7)+1}行第${i%7+1}列"></button>`).join('')}</div><div class="robot-pointer" id="robot-pointer" aria-hidden="true">🤖<span></span></div></div>${editing?`<div class="editor-save"><button id="opening-cancel">取消</button><button class="primary" id="opening-apply">保存并从这里开始</button></div>`:`<div class="robot-player"><button class="icon-button" id="robot-reset" aria-label="从初始棋盘重新开始" title="重新开始">↺</button><button class="primary play" id="robot-play">▶ 机器人开玩</button><button class="icon-button" id="robot-step" aria-label="执行一步" title="执行一步">▷|</button><button class="icon-button" id="robot-undo" aria-label="撤销一步" title="撤销一步">↶</button></div><div class="robot-speed" role="group" aria-label="播放速度">${[.5,1,2,4,8,16,32].map(n=>`<button data-robot-speed="${n}" class="${n===speed?'active':''}" aria-pressed="${n===speed}">${n}×</button>`).join('')}</div>`}<div class="robot-caption"><span>${editing?'拖动可换位置；底部生成器固定':'7 × 8 · 合成免费，生成和加工扣体力'}</span><button id="robot-rule-link">规则 ⓘ</button></div><div class="robot-selection" id="robot-selection"></div></section><aside class="robot-side">${editing?editorPanel():`<section class="panel strategy-panel"><div class="panel-head"><h2>机器人先做什么</h2><span class="live-label">下一步生效</span></div><div class="panel-body"><div class="strategy-grid">${Object.entries(STRATEGIES).map(([key,s])=>`<button data-strategy="${key}" class="${key===policy?'active':''}" aria-pressed="${key===policy}"><span>${s.icon}</span>${s.name}</button>`).join('')}</div><div class="strategy-reason" id="strategy-reason"></div><label class="focus-row">优先物品线<select id="robot-focus" aria-label="优先物品线"><option value="auto">机器人选择</option>${OS.map((k,i)=>`<option value="${k}" ${k===focus?'selected':''}>${ICONS[i]} ${osLabel(k)}</option>`).join('')}</select></label></div></section><section class="panel orders-panel"><div class="panel-head"><h2>订单</h2><b class="live-label" id="robot-progress"></b></div><div class="panel-body" id="robot-orders"></div></section><section class="panel monitor-panel"><div class="panel-head"><h2>这一局</h2><button class="small" id="robot-refill" title="手动补充100体力">ϟ +100</button></div><div class="panel-body"><div class="robot-monitor" id="robot-monitor"></div><div class="energy-caption"><span>剩余体力</span><span id="robot-time"></span></div><div id="robot-energy-chart"></div><div class="energy-caption"><span>棋盘空间</span><span id="robot-space-count"></span></div><div class="space-meter" id="robot-space"></div><div class="robot-event" id="robot-event" role="status" aria-live="off"></div><details><summary>操作记录与手动加工</summary><div id="robot-log" class="robot-log"></div><div id="robot-craft" class="robot-manual"></div></details></div></section><section class="panel"><div class="panel-body" style="padding-top:16px"><label class="focus-row" style="margin-top:0">开局<select id="robot-opening" aria-label="初始棋盘来源"><option value="empty" ${opening==='empty'?'selected':''}>空棋盘</option><option value="carry" ${opening==='carry'?'selected':''}>带入上一单库存</option><option value="custom" ${opening==='custom'?'selected':''} ${savedOpening()?'':'disabled'}>我保存的棋盘</option></select></label></div></section>`}</aside></div></div>`;
  bind();paint();if(editing)paintPalette();
}
function paintStatus(){
  ...
  $('#robot-current-action').textContent=editing?'保存后开始一局新模拟':`${STRATEGIES[policy].name} · ${sim.actions} 步`;
...
function paint(){
  ...
  paintOrders();paintMonitor();paintSelection();
}
function paintOrders(){
  const stock=sim.stock(),o=sim.current();$('#robot-progress').textContent=`${sim.delivered} / ${sim.orders.length}`;
  $('#robot-orders').innerHTML=`<div class="order-progress">${sim.orders.map((_,i)=>`<span class="${i<sim.cursor?'done':i===sim.cursor?'current':''}"></span>`).join('')}</div>`+(sim.complete()?`<div class="complete-banner"><span>✓</span><h3>全部交付</h3><p>${sim.actions} 步 · ${fmt(sim.spent)} 体力</p><button class="primary" id="robot-next-day">下一天 →</button></div>`:`<div class="split"><b style="font-size:.875rem">第 ${o.order} 单</b>${o.clean?'<span class="pill">♻ 库存回收</span>':''}</div>${o.lines.map(l=>{const item=byName.get(l.name),have=Math.min(l.quantity,stock[l.name]||0);return `<div class="order-need ${have===l.quantity?'ready':''}"><span class="item-glyph">${glyph(item)}</span><div class="need-copy"><button data-robot-item="${esc(l.name)}">${esc(item?.display??l.name)}</button><div class="mini-track"><span style="width:${have/l.quantity*100}%;background:#30ad97"></span></div></div><b class="amount">${have===l.quantity?'✓':`${have} / ${l.quantity}`}</b></div>`}).join('')}<button id="robot-submit" class="full ${sim.indicesFor(sim.demands())?'primary':''}" ${sim.indicesFor(sim.demands())?'':'disabled'}>交付订单</button>${sim.orders[sim.cursor+1]?`<div class="robot-next"><span>下一单</span>${sim.orders[sim.cursor+1].lines.map(l=>`<span class="item-glyph" title="${esc(byName.get(l.name)?.display??l.name)}">${glyph(byName.get(l.name))}</span>`).join('')}</div>`:''}`);
  document.querySelectorAll('[data-robot-item]').forEach(b=>b.onclick=()=>itemDetail(b.dataset.robotItem));
  if($('#robot-submit'))$('#robot-submit').onclick=()=>manual(()=>sim.execute(sim.describe({type:'submit'})));
...
function paintMonitor(){
  $('#robot-monitor').innerHTML=`<div><b>${fmt(sim.spent)}</b><small>消耗体力</small></div><div><b>${sim.actions}</b><small>操作步数</small></div><div><b>${sim.merges}</b><small>合成次数</small></div>`;
  $('#robot-time').textContent=`模拟 ${clock(sim.elapsed)}`;
  ...
  $('#robot-energy-chart').innerHTML=`<svg class="energy-chart" viewBox="0 0 320 66" role="img" aria-label="剩余体力随操作步数变化"><defs><linearGradient id="energy-fill" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#6a8bec" stop-opacity=".25"/><stop offset="1" stop-color="#6a8bec" stop-opacity=".02"/></linearGradient></defs><path d="M4,60 L${points.map(xy).join(' L')} L${4+points.at(-1).step/last*312},60 Z" fill="url(#energy-fill)"/><path d="M${points.map(xy).join(' L')}" fill="none" stroke="#6587ed" stroke-width="2.5"/></svg>`;
  ...
  const event=$('#robot-event');event.textContent=sim.failure?sim.energy<1?'体力用完了，补充后继续':sim.failure:sim.lastAction?`${({generate:'⚡',merge:'🧩',craft:'🛠',submit:'📦'})[sim.lastAction.type]} ${sim.lastAction.label}${sim.lastAction.cost?' · −'+fmt(sim.lastAction.cost)+' 体力':''}`:sim.actions?sim.log[0]:'点播放，看机器人完成这一局';event.classList.toggle('error',!!sim.failure);
  $('#robot-log').innerHTML=sim.log.slice(0,12).map(s=>`<p>${esc(s)}</p>`).join('');
...
function paintSelection(){let item=byName.get(sim.cells[selected]?.name);$('#robot-selection').innerHTML=item?`<span>${glyph(item)} ${esc(item.display)}</span><button id="selected-detail">配方</button><button id="selected-remove">移出</button>`:'也可以自己点击生成、拖动合成';if(item){$('#selected-detail').onclick=()=>itemDetail(item.name);$('#selected-remove').onclick=()=>manual(()=>sim.remove(selected))}}
...
function manual(fn){stopRobot();const r=fn();if(r?.ok===false)toast(r.message);selected=-1;paint()}
function showRules(){
  $('#detail-body').innerHTML=`<h2>棋盘怎么玩</h2><div class="model-help"><p>⚡ 生成一次扣1体力；同级合成免费；加工按配置扣体力。体力用完暂停，补充按钮每次加100。</p><p>📦 按配置顺序完成订单。策略决定先准备什么材料，不跳单；没有不同合法动作时，切换策略仍可能走同一步。</p><p>▶ 1×按生成0.8秒、合成0.65秒、加工1.1秒、交付1.2秒播放。速度只影响观看，计时使用这套演示节拍。</p><p>当前使用配置效率的期望产出，尚未接入随机掉落、冷却和自然恢复。礼盒可手动生成，回收规则也作了棋盘化处理，因此体力可能与表格连续计算不同。</p><p>初始布局只保存在当前浏览器；保存布局会开始一局新模拟。原始配置不变。</p></div>`;$('#detail').showModal();
}
...
function applyEditor(){
  ...
  stopRobot();sim=new RobotSimulator(D,state.day,{layout:draft,energy:value,strategy:policy,focus});initialLayout=sim.exportLayout();initialEnergy=sim.energy;opening='custom';editing=false;
...
function bind(){
  ...
  document.querySelectorAll('[data-robot-speed]').forEach(b=>b.onclick=()=>{speed=+b.dataset.robotSpeed;player.setSpeed(speed)});document.querySelectorAll('[data-strategy]').forEach(b=>b.onclick=()=>changeRobotPolicy(b.dataset.strategy,focus));$('#robot-focus').onchange=e=>changeRobotPolicy(policy,e.target.value);
